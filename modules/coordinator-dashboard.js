  ...
  try {
    ...
    el.innerHTML = `
      ...
      <div style="overflow-x:auto;">
        <table>
          <thead>
            <tr><th>Name</th><th>E-Mail</th><th>Rollen</th><th>Aktionen</th></tr>
          </thead>
          <tbody>
            ${users.map(u => `
              <tr>
                ...
                <td style="white-space:nowrap;">
                  ...
                  <button class="btn-danger" data-user-id="${u.id}" data-action="delete-user" style="padding:4px 10px;margin-left:4px;">Löschen</button>
                </td>
              </tr>
            `).join('')}
          </tbody>
    ...
    el.querySelectorAll('[data-action="delete-user"]').forEach(btn => {
      btn.onclick = () => confirmDeleteUser(users.find(u => u.id === btn.dataset.userId), el);
    });
  } catch (e) {
    console.error(e);
...
function confirmDeleteUser(user, parentEl) {
  ...
  showModal({
    ...
    body: `
      <p>Soll der Benutzer <strong>${user.displayName || user.email || user.id}</strong> gelöscht werden?</p>
      <p class="text-muted">Das löscht den Firestore-Benutzer, entfernt ihn aus Gruppen und aus Trainer-Zuweisungen. Der Firebase-Auth-Account selbst bleibt bestehen, da das clientseitig nicht sicher gelöscht werden kann.</p>
    `,
    ...
    onConfirm: async () => {
      try {
        ...
        const attendanceSnap = await firestore.collection('eventAttendance').where('userId', '==', user.id).get();
        attendanceSnap.forEach(doc => batch.delete(doc.ref));

        await batch.commit();
        showToast('Benutzer gelöscht (Firestore).', 'success');
        renderUsersTab(parentEl);
      } catch (e) {
        console.error(e);
...
async function showGroupMembersDialog(group, parentEl) {
  ...
  overlay.innerHTML = `
    <div class="members-dialog" style="
      ...
      <div style="padding:20px 24px 16px; border-bottom:1px solid var(--color-border); display:flex; justify-content:space-between; align-items:center;">
        ...
        <button id="mgd-close" style="background:none;border:none;font-size:1.4rem;color:var(--color-text-muted);cursor:pointer;padding:4px 8px;border-radius:4px;">&times;</button>
      </div>
  ...
  function updateCounter() {
    counterEl.textContent = `${selected.size} / ${availableUsers.length} ausgewählt`;
  }
  ...
  function renderList(filter = '') {
    ...
    filtered.forEach(u => {
      ...
      Object.assign(row.style, {
        display: 'flex', alignItems: 'center', gap: '14px', padding: '10px 12px', borderRadius: '8px', cursor: 'pointer',
        transition: 'background 0.1s', marginBottom: '2px', backgroundColor: isChecked ? 'rgba(21,101,192,0.08)' : 'transparent'
      });
      row.innerHTML = `
        <input type="checkbox" data-uid="${u.id}" ${isChecked ? 'checked' : ''} style="width:18px;height:18px;flex-shrink:0;" />
      ...
      cb.onchange = () => {
        if (cb.checked) {
          selected.add(u.id);
          row.style.backgroundColor = 'rgba(21,101,192,0.08)';
        } else {
  ...
  overlay.querySelector('#mgd-select-all').onclick = () => {
    availableUsers.forEach(u => selected.add(u.id));
...
function renderEventList(el, events, groups, parentEl, deleteSelBtn) {
  if (!events.length) {
    el.innerHTML = '<p class="text-muted">Keine Termine vorhanden.</p>';
    return;
  }
  ...
  el.innerHTML = `
    <div style="overflow-x:auto;">
      <table id="events-table">
        <thead>
          <tr>
            <th style="width:36px;"><input type="checkbox" id="select-all-events" title="Alle auswählen" style="width:16px;height:16px;" /></th>
          ...
          ${sorted.map(ev => {
            ...
            return `
              <tr data-event-id="${ev.id}">
                <td><input type="checkbox" class="event-select-cb" data-id="${ev.id}" style="width:16px;height:16px;"></td>
                <td>${ev.title || '–'}</td>
  ...
  el.querySelectorAll('.event-select-cb').forEach(cb => {
    cb.onchange = () => {
      const checkboxes = [...el.querySelectorAll('.event-select-cb')];
  ...
  showModal({
    ...
    onConfirm: async () => {
      ...
      for (let i = 0; i < toDelete.length; i += 499) {
        const batch = firestore.batch();
        toDelete.slice(i, i + 499).forEach(e => batch.delete(firestore.collection('events').doc(e.id)));
        await batch.commit();
      }
...
function renderCalendarView(el, events, groups, parentEl) {
  ...
  for (let i = 0; i < startWd; i++) calHtml += `<div style="min-height:90px;"></div>`;
  ...
  for (let day = 1; day <= lastDay.getDate(); day++) {
    ...
    const evPills = dayEvents.slice(0, 3).map(ev => `
      <div class="cal-event-pill ${ev.status === 'cancelled' ? 'cancelled' : ''}" data-ev-id="${ev.id}"
        style="font-size:0.72rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;border-radius:3px;padding:1px 5px;margin-bottom:1px;cursor:pointer;background:${ev.status === 'cancelled' ? 'var(--color-error)' : 'var(--color-primary)'};color:#fff;">
        ${ev.title || 'Termin'}
      </div>
    `).join('');
    ...
    calHtml += `
      <div style="min-height:90px;border:1px solid var(--color-border);border-radius:6px;padding:4px;background:${isToday ? 'rgba(21,101,192,0.07)' : 'var(--color-surface)'};position:relative;">
...
function showEventForm(event, groups, parentEl) {
  ...
  showModal({
    ...
    body: `
      ...
      <label>Trainer-UIDs (kommagetrennt, optional)</label>
